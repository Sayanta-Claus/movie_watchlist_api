...
//seed file to populate db with fake data to test api

//this user will create all the fake movies
const userId = "020ca62d-b0eb-4870-95af-bebd8af09f09";
...
//seeds db with movies from above array
const main = async () => {
...
//run main func and since it is async catch any errors
main()
  .catch((err) => {
    console.error(err);
    process.exit(1);
  ...
  .finally(async () => {
    await prisma.$disconnect(); // to prevent memory leaks
  });

//we created a seed script for this file in package.json
